...
static unsigned int almk_swap_ratio2 = 6;
...
/*huacai.zhou@PSW.BSP.Kernel.MM, 2018/03/12, use aggressive for lowmem*/
		if (almk_enable && (swap_pages * almk_swap_ratio1 < total_swap_pages))
			(*to_be_aggressive)++;
		if (almk_enable && (swap_pages * almk_swap_ratio2 < total_swap_pages))
			(*to_be_aggressive)++;
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
/*tianwen@PSW.BSP.Memory, 2019-04-26, check free and file pages before almk*/
		if (*to_be_aggressive > 0 && other_file < totalram_pages/almk_totalram_ratio) {
			i -= *to_be_aggressive;
			if (likely(i >= 0))
...
/*tianwen@PSW.BSP.Memory, 2019-04-26, check free and file pages before almk*/
	...
	for_each_process(tsk) {
		...
		if (tsk->flags & PF_KTHREAD)
			continue;
#ifdef ODM_WT_EDIT
/*duyingying@ODM_WT.BSP.Kernel, 2020/07/31, mtk_lowmemorykiller jump kill 'autotest.monkey'*/
	        if((strstr(tsk->comm,"autotest.monkey"))!=NULL){
                   lowmem_print(1,"mtk_lowmemorykiller jump kill 'autotest.monkey' \n");
                   continue;
                 }
#endif
